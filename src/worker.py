# biblioteca padrão do Python para lidar com sistema de arquivos
import os
# biblioteca de mensageria RabbitMQ
import pika
# Lê as mensagens da fila e transforma elas em dicionário python
import json
# Conecta o Worker ao mesmo banco de memória que a API usa
# Permite atualizar o status, dizendo ao Front que o PDF está pronto
import redis
# Utilizada para simular tempo de processamento
import time
# Utilizada para capturar a data e o horário da geração do relatório
from datetime import datetime
# Utilizado para que a data e o horário saiam no fuso horário de Brasília
import pytz
# Biblioteca para geração dos PDFs
...
# Função criada para disparar automaticamente toda vez que uma mensagem chega na fila
def callback(ch, method, properties, body):
    dados    = json.loads(body) # Transforma o texto recebido em dados utilizáveis pelo Python
    task_id  = dados['task_id'] 
    cidade   = dados['cidade'].title() # Padroniza para maiúscula a primeira letra de cada palavra
    temp     = round(dados['temperatura']) # Arredonda a temperatura para um número inteiro para ficar esteticamente melhor no PDF.

    print(f"[*] Gerando guia para {cidade} ({temp}°C)...")
    time.sleep(3)

    #O Worker analisa a temperatura recebida da API e decide quais textos e cores serão utilizados no PDF.
    if temp >= 28:
        faixa     = "Clima Quente"
